...
- **Golden 语料：164 条，143 match（87.2%），21 gap（12.8%）。**
- **全量测试：513 条，全部通过。**
- **定向 phonemize 测试：143 条，全部通过。**
- 中文 G2P 已对齐一批 `misaki` v1.1 `ZHFrontend` 注音、变调和儿化行为，并已补齐核心 `HH:MM` / 时间范围、`YYYY-MM-DD` / `YYYY/MM/DD` 日期、数字范围、分数、负整数、带符号百分比、小数边界、电话号码、温度/常见单位、`299元` / `4个半小时` / 通用量词数字读法，以及当前 P1 范围内的短英文策略、长句分词边界、三声/轻声组合和数字序列分词。
...
- [x] **量词数字核心读法（部分）** (`kokoro.js/src/phonemize.js`, `kokoro.js/tests/phonemize.test.js`)
  ...
  - 定向测试更新到 118/118 通过
- [x] **量词数字长尾** (`kokoro.js/src/phonemize.js`, `kokoro.js/src/zh-data.js`, `kokoro.js/tests/phonemize.test.js`)
  - 引入 `CHINESE_QUANTIFIERS`，覆盖常见量词、时间单位、度量衡和金额单位
  - `买3本书`、`距离12公里`、`价格10086元`、`预算1000000元` 走 `RE_POSITIVE_QUANTIFIERS` 风格归一化
  - 保持 `电话10086` 服务号读法，并避免 `编号10086` 被误读为电话或金额
  - 定向测试更新到 133/133 通过
...
| 范围/分数/负数/百分比/小数边界 | **164** | **130** | **34** | 历史 |
| P0/P1 收敛（电话、量词、长句边界、三声/轻声、年份序列） | **164** | **143** | **21** | 当前 |
...
| `299元` / `239元` / `1000000元` | 金额和量词数字读法 ✓ | 所在长句仍有 `这件`、`原价` 等非数字分词差异 |
...
| 电话 | `RE_MOBILE_PHONE` / `RE_TELEPHONE` | 手机号、座机、400 号和电话上下文服务号已补齐 |
...
- [x] **电话号码**：已移植核心 `RE_MOBILE_PHONE` / `RE_TELEPHONE` / `RE_NATIONAL_UNIFORM_NUMBER` 行为，覆盖手机号、座机、400 号，并用电话上下文服务号替代 `10086` 特例硬编码。
- [x] **温度和度量衡核心项**：已覆盖 `36.5℃` / `36.5°C`、`3kg`、`10cm` 等真实输入；更多单位作为长尾扩展。
- [x] **量词数字长尾**：已系统移植 `RE_POSITIVE_QUANTIFIERS` 风格规则，覆盖更多单位/量词组合；长句中剩余差异主要来自非数字分词边界。

验收方式：每条规则新增 `phonemize.test.js` 定向用例，并把对应 `zh-misaki-golden.json` 中可对齐样例从 `gap` 改为 `match`。

### P1：收敛现有 golden gap

- [x] **重新统计 gap 列表**：当前 fixture 实际有 21 个 `gap`，`当前状态` 已更新为 143 match / 21 gap。
- [x] **短英文/缩写策略**：保留 JS 英文 phonemization，用于中英混合 TTS 的可读输出；不严格复刻 Python `unk=❓`，相关 golden 样例继续作为已知策略差异保留 `gap`。
- [x] **长句分词边界**：已补一批有限 Segmenter 后处理（`我想去`、`上海的`、`买了`、`吃了`、`做的`、`热腾腾的` 等），并避免误伤 `新城地标`、`为了孩子`、`吃了饭`。
- [x] **三声/轻声组合残留**：已补 `试试看` / `能不能` / `一举两得` / `学不好` / `一分为二地看` 等当前高价值残留；剩余差异暂不继续追平。
- [x] **数字序列分词**：已处理 `1980年`、`2008年8月8日`、`2023年`、`2024年`、`2025年12月31日` 等年份/日期序列；金额/量词数字读法已补，剩余 GDP/iPhone 差异属于短英文策略。

### P2：决定分词路线

- [x] **暂跳过**：当前继续使用 `Intl.Segmenter` + 小规模后处理；不引入 `jieba-wasm`、`segmentit` 或其它分词依赖。
- [x] **保守路线**：覆盖 demo 和常见文本，不追求完全复刻 Python `jieba.posseg`。
- [x] **浏览器成本评估**：暂不做；只有后续明确要继续提高 golden match rate 时再评估包体积、初始化耗时、Worker 兼容性和离线可用性。
...
## 待定事项
- 是否引入浏览器友好的中文分词依赖（如 `jieba-wasm`），替代目前规则驱动的 Intl.Segmenter 后处理（当前跳过）
- 是否把 Python `TextNormalizer` 视为必须对齐目标；当前 Python `ZHG2P` 主路径只走 `cn2an.transform()`，并没有调用完整 `zh_normalization`
- 是否在 demo 里暴露运行时 model 选择器（当前不做，model 固定；voice 仍可选择）
